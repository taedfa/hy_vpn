...
inst_port(){
    ...
    yellow "将在 Hysteria 2 节点使用的固定端口是：$port"
    inst_jump
}
...
inst_site(){
    ...
    [[ -z $proxysite ]] && proxysite="www.bing.com"   #en.snu.ac.kr
...
EOF
    ...
    yellow "Hysteria 2 节点二维码如下，使用客户端工具扫描添加节点。"
    qrencode -t ANSIUTF8 < /root/hy/url.txt
}
